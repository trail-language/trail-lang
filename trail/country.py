"""Country-code normalization to ISO 3166-1 alpha-3.

`meta.country` is canonically ISO3 (matching country-keyed sources like GMD). Providers
receive countries in mixed forms - alpha-2 ("US"), alpha-3 ("USA"), or already normalized -
so :func:`to_iso3` maps them to a single canonical alpha-3, returning ``None`` for anything
unrecognized (an unmapped country yields a null bridge, which nulls that entity's macro).
"""
...
_ISO3 = frozenset(_ISO2_TO_ISO3.values())


def to_iso3(value: str | None) -> str | None:
    """Normalize a country code (alpha-2 or alpha-3) to canonical ISO3, or None if unknown."""
    if not value:
        return None
    v = str(value).strip().upper()
    if len(v) == 3:
        return v if v in _ISO3 else None
    return _ISO2_TO_ISO3.get(v)
